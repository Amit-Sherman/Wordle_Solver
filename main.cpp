...
int main()
{
    ...
    auto first_best_word = best_words_to_guess(words_dataset)[0];
    ...
    for(int i = 0; i < NUM_OF_GAMES; i++)
    {
        ...
        while(possible_words.size() > 1)
        {
            ...
            else best_word = best_words_to_guess(possible_words)[0];
            std::cout << "Guessing the word: " << best_word.first << ", with average information gain of: " << best_word.second << std::endl;
            int feedback_pattern = translate_to_pattern(feedback(best_word.first, target));
